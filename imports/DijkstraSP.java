 ...
 *  Assumes all capacitys are nonnegative.
 *
 ...
 * single-source shortest paths problem in edge-capacityed digraphs where the
 * edge capacitys are nonnegative.
 * <p>
...
public class DijkstraSP {
     ...
     * other vertex in the edge-capacityed digraph {@code G}.
     *
     * @param G the edge-capacityed digraph
     * @param s the source vertex
     * @throws IllegalArgumentException if an edge capacity is negative
     * @throws IllegalArgumentException unless {@code 0 <= s < V}
     */
    public DijkstraSP(FlowNetwork G, int s) {
        for (FlowEdge e : G.edges()) {
            if (e.capacity() < 0) {
                throw new IllegalArgumentException("edge " + e + " has negative capacity");
            }
        }
    ...
    // check optimality conditions:
    // (i) for all edges e:            distTo[e.to()] <= distTo[e.from()] + e.capacity()
    // (ii) for all edge e on the SPT: distTo[e.to()] == distTo[e.from()] + e.capacity()
    private boolean check(FlowNetwork G, int s) {

        // check that edge capacitys are nonnegative
        for (FlowEdge e : G.edges()) {
            if (e.capacity() < 0) {
                System.err.println("negative edge capacity detected");
                return false;
            }
        }
        ...
        // check that all edges e = v->w satisfy distTo[w] <= distTo[v] + e.capacity()
        for (int v = 0; v < G.V(); v++) {
            for (FlowEdge e : G.adj(v)) {
                int w = e.to();
        ...
        // check that all edges e = v->w on SPT satisfy distTo[w] == distTo[v] + e.capacity()
        for (int w = 0; w < G.V(); w++) {
            if (edgeTo[w] == null) {
                continue;
            }
